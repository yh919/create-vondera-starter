...
async function replaceVariables(projectName, targetDir, apiKey, storeName) {
  ...
  for (const filePath of allFiles) {
    if (await fs.pathExists(filePath)) {
      ...
      if (
        filePath.match(/\.(jpg|jpeg|png|gif|svg|webp|ico|woff|woff2|ttf|eot)$/i)
      )
        continue;
...
function initGit(targetDir) {
  try {
    ...
    execSync('git commit -m "Initial commit from create-vondera-starter"', {
      cwd: targetDir,
      stdio: "ignore",
    });
  } catch (e) {
